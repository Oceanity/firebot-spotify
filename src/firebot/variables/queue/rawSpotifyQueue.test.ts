import { testQueue, testTrigger } from "@/test-data";
...
describe("Spotify - Raw Queue Replace Variable", () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(spotify.player.queue, "getAsync").mockResolvedValue(testQueue);
  });
  ...
  it("returns empty string when queue is undefined", async () => {
    jest.spyOn(spotify.player.queue, "getAsync").mockResolvedValue(null);
